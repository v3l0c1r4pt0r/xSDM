...
START_TEST (test_check_windatetounix)
{
    uint64_t target[] = {
        0x01ce1bc45a4a92b8,
        0x01cc1efe64182b6f,
    ...
    int i;
    for(i = 0; i < sizeof(target)/sizeof(uint64_t); i++)
    {
        uint64_t actual = winTimeToUnix(target[i]);
...
xsdc_suite (void)
{
    ...
    tcase_add_test (tc_core, test_check_windatetounix);
